...
class ERC20Token:
    def __init__(self, address: str):
        ...
        except (exceptions.BadFunctionCallException, exceptions.ContractLogicError) as e:
    ...
    def getAllowance(self, spender: ChecksumAddress):
        try:
            ...
            print(f'Error retrieving allowance: {e}')
            raise

    def approve(self, spender: ChecksumAddress, amount: Decimal):
        try:
            current_allowance = self.getAllowance(spender)
        except exceptions.ContractLogicError as e:
            print(f"Error checking allowance: {e}")
            return False

        required_allowance = chain_helper.to_raw_value(amount, self.decimals)
        ...
        try:
            ...
            print(f"Error in approve transaction: {e}")
            return False
        except Exception as e:
